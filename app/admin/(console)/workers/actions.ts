...
export async function saveWorker(input: { id?: string; code: string; name: string; pin: string; active: boolean; role_id?: string }): Promise<ActionResult> {
  ...
  const row = { code: input.code.trim(), name: input.name.trim(), pin: input.pin, active: input.active, role_id: input.role_id || "general" };
  if (input.id) {
